...
def main():
    ...
    # Get train parameters
    best_tr_tp = [tr_tp[i] for i in param_idxs[::-1][:args.best]]
    best_tr_tn = [tr_tn[i] for i in param_idxs[::-1][:args.best]]
    best_tr_fp = [tr_fp[i] for i in param_idxs[::-1][:args.best]]
    best_tr_fn = [tr_fn[i] for i in param_idxs[::-1][:args.best]]
    best_tr_acc = [tr_acc[i] for i in param_idxs[::-1][:args.best]]
    best_tr_pre = [tr_pre[i] for i in param_idxs[::-1][:args.best]]
    best_tr_rec = [tr_rec[i] for i in param_idxs[::-1][:args.best]]
    best_tr_fpr = [tr_fpr[i] for i in param_idxs[::-1][:args.best]]
    best_tr_fsc = [tr_fsc[i] for i in param_idxs[::-1][:args.best]]

    # Get test parameters
    best_tst_ev_tm = [tst_ev_tm[i] for i in param_idxs[::-1][:args.best]]
    best_tst_tp = [tst_tp[i] for i in param_idxs[::-1][:args.best]]
    best_tst_tn = [tst_tn[i] for i in param_idxs[::-1][:args.best]]
    best_tst_fp = [tst_fp[i] for i in param_idxs[::-1][:args.best]]
    best_tst_fn = [tst_fn[i] for i in param_idxs[::-1][:args.best]]
    best_tst_acc = [tst_acc[i] for i in param_idxs[::-1][:args.best]]
    best_tst_pre = [tst_pre[i] for i in param_idxs[::-1][:args.best]]
    best_tst_rec = [tst_rec[i] for i in param_idxs[::-1][:args.best]]
    best_tst_fpr = [tst_fpr[i] for i in param_idxs[::-1][:args.best]]
    best_tst_fsc = [tst_fsc[i] for i in param_idxs[::-1][:args.best]]
