...
def main():
    ...
    evaluate_dataset(das_seis_loader, 'DAS_seismic', device,
                     net, args.model_name, args.model_folder,
                     '../Results/Testing/Outputs')

    # Evaluate model on STEAD seismic test set
    evaluate_dataset(das_nseis_loader, 'DAS_non_seismic', device,
                     net, args.model_name, args.model_folder,
                     '../Results/Testing/Outputs')

    # Evaluate model on STEAD seismic test set
    evaluate_dataset(das_noise_loader, 'DAS_noise', device,
                     net, args.model_name, args.model_folder,
                     '../Results/Testing/Outputs')
...
def evaluate_dataset(data_loader, dataset_name, device, net,
    ...
    Path(f"../Results/Classified/DAS_sep/{dataset_name}/"
         f"{model_name}_({thr})/Worst_seis/").mkdir(parents=True, exist_ok=True)

    Path(f"../Results/Classified/DAS_sep/{dataset_name}/"
         f"{model_name}_({thr})/Best_nseis/").mkdir(parents=True, exist_ok=True)
    ...
    with tqdm.tqdm(total=len(data_loader),
        ...
        with torch.no_grad():
            for data in data_loader:
                ...
                for out, tr, lab in zip(outputs, traces, labels):
                    if lab == 1 and out.item() < 0.1 and i < 500:
                        plt.clf()
                        plt.plot(tr.cpu())
                        plt.xlabel('Samples')
                    ...
                    elif lab == 0 and out.item() > 0.9 and j < 500:
                        plt.clf()
                        plt.plot(tr.cpu())
                        plt.xlabel('Samples')
                        plt.ylabel('Amplitude')
